...
Hier präzisieren wir unseren Ansatz aus Sektion \ref{sec:osz1} auf Seite \pageref{sec:osz1}.
Wir gehen nun davon aus, dass wir hochfrequente Oszillationen Gitterebene vorhanden sind und untersuchen, wie sich die Amplitude in der Zeit ausbreitet.
Daher wählen wir für $u_1$ und $u_2$ aus \eqref{eq:adv:osz:ansatz} nun konkreter 
\[ 
u_1(n, i, t_n, x_i) + h u_2(n, i, t_n, x_i) 
...
\end{align}
Da \eqref{eq:osz2:n1} und \eqref{eq:osz2:n} beide gleich sind, folgt unter der Annahme, dass $\Psi_1(\lambda h, h) \neq 0$ gilt
\begin{align}\label{eq:osz:omegan}
A (-1)^n = \Omega(n+1) - ( 1 - 2 \lambda ) \Omega(n)
\end{align}
Und das impliziert wiederum
\begin{align}\nonumber
\Omega(n+1) &= A \sum_{k=0}^n (-1)^{n-k} (1 - 2 \lambda)^k + (1 - 2 \lambda)^n\\
&= A (- 2 \lambda)^n + (1 - 2 \lambda)^n.
\end{align}
Hieraus folgt für $2 \lambda > 1$, also $2\lambda = 1 + K$ für ein $K > 0$
\begin{align*}
\abs{\Omega(n+1)} &= \abs{A (- 2 \lambda)^n + (1 - 2 \lambda)^n}\\
&\geq \bigl\lvert \abs{A (- 2 \lambda)^n} - \abs{(1 - 2 \lambda)^n} \bigr\rvert\\
...
\begin{align*}
0 = \lim_{h \to 0} \: h \frac{\abs{u_1(n, i, t_n, x_i)}}{\abs{u_0(t_n, x_i)}}
...
\end{align*}
Und hieraus folgern wir nun $A = 0$ und mit \eqref{eq:osz:omegan} gilt für alle $n \in \N$
