...
\end{lemma}
\begin{proof}
Wie in Lemma~\ref{lemma:transport:diskret:konstant1} setzen wir $h = \frac {t}{\lambda n}$.
...
\begin{align*}
u_0(0, i, 0, x_i) &= v^0_i = \rd\bigl(U(x_i)\bigr) = U(x_i) - \bigl[ \underbrace{U(x_i) - \rd\bigl(U(x_i)\bigr)}_{=: \Delta_\eps U(x_i)} \bigr]\\
...
\end{align}
und sortiert man die Gleichungen \eqref{eq:transport:diskret:oh} und \eqref{eq:transport:diskret:oh2} nach stetigen und unstetigen Anteilen, so erhalten wir die Gleichungen

\vspace{0.4cm}
\noindent \textbf{In der Ordnung $O(h)$:}
\begin{align}\label{eq:transport:osz:oh}
\partial_t w_0(t_n, x_i) + \partial_x w_0(t_n, x_i) = 0\\
...
\partial_t z_1(t_n, x_i) + \partial_x z_1(t_n, x_i) &= \frac {\lambda - 1} {2 (2\lambda - 1)}   \partial^2_x z_0(t_n, x_i)
\end{align}
