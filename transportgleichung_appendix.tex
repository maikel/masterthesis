...
\begin{split}
F(t_n, x_k) &= e^{i \omega k} f(t_n, x_k) \qquad \text{bzw.}\\
F(t_n, x_k) &= \Ree\left( e^{i \omega k} f(t_n, x_k) \right)
\end{split}
\end{align}
...
\begin{align*}
F(t_n, x_k) &= e^{i \omega k} f(t_n, x_k)\\
&\text{und}\\
F(t_n, x_{k+1}) &= e^{i \omega (k + 1)} f(t_n, x_{k+1})\\
&= e^{i \omega k} e^{i \omega} f(t_n, x_{k+1})\\
&= e^{i \omega k} e^{i \omega} \bigl( f(t_n, x_k) + h \partial_x f(t_n, x_k) + o(h) \bigr)
\end{align*}
also gilt für alle $h > 0$ auch
\begin{align}
\begin{split}\nonumber
\abs{F(t_n, x_k + h) - F(t_n, x_k)} &= \abs{F(t_n, x_{k+1}) - F(t_n, x_k)}\\
&= \abs{e^{i \omega k} (e^{i \omega} - 1) f(t_n, x_k) + h e^{i \omega k} \partial_x f(t_n, x_k)}\\
&\leq \abs{ e^{i \omega k} (e^{i \omega} - 1) f(t_n, x_k) }  + h  \abs{ e^{i \omega k} \partial_x f(t_n, x_k) }\\
&= \underbrace{\abs{ e^{i \omega k} }}_{= 1} \abs{ (e^{i \omega} - 1) f(t_n, x_k) } + h  \underbrace{\abs{ e^{i \omega k} }}_{= 1} \abs{ \partial_x f(t_n, x_k) }
\end{split}\\ \label{satz:glgregulaer:bed}
&= \underbrace{\abs{e^{i \omega} - 1}}_{\neq 0} \abs{f(t_n, x_k)} + h \abs{\partial_x f(t_n, x_k)}.
\end{align}
...
Es gilt weiterhin
\[ \lim_{p \to \infty} \abs{e^{i \omega} - 1} \abs{f(t^p, x^p)} + h_p \abs{\partial_x f(t^p, x^p)} = \lim_{p \to \infty} \abs{e^{i \omega} - 1} \abs{f(t^p, x^p)}. \]
...
\[ f(t,x) = \lim_{p \to \infty} f(t^p, x^p) = 0 \qquad \text{für } h \to 0 \: \text{und} \: (t^p, x^p) \to (t,x), \]
was zu zeigen war.
\end{proof}