...
\partial_t z_1(t, x) + \frac{1}{2 \lambda - 1} \partial_x z_1(t, x) &= \frac{\lambda - 1}{2 (2 \lambda - 1)^2} \partial^2_x z_0(t, x) \label{eq:transport:osz:oh2_B}
\end{align}

\noindent \textbf{Mit den Anfangsbedingungen in \eqref{eq:transport:osz:anfangsbedinungen}}

\vspace{0.4cm}
Betrachtet man den Ansatz \eqref{eq:transport:osz:ansatz} nun genauer, so erkennt man, dass der Faktor $(1 - 2\lambda)^n$ für $\lambda = 1 + \eta$ und $n \to \infty$ unbeschränkt ist.
...
w_1(t, x) &= t \frac{1 - \lambda} 2 \partial^2_x U(x - t)
\end{align}
akzeptieren.
...
\begin{align*}
0 &\leq \lim_{n \to \infty} \abs{\frac{w_1(t_n,x_i)}{(1 - 2 \lambda)^n z_0(t_n, x_i)}}\\
&= \lim_{n \to \infty} \abs{\frac{t_n \frac{1 - \lambda} 2 \partial^2_x U(x_i - t_n)}{(1 - 2 \lambda)^n z_0(t_n, x_i)}}\\
&= \lim_{n \to \infty} \abs{\frac{n \lambda h (1 - \lambda) \partial^2_x U(x_i - t_n)}{2 (1 - 2 \lambda)^{n} z_0(t_n, x_i)}}\\
&\leq \frac{h (1 - \lambda)}{C} \norm{\partial^2_x U}_\infty \cdot \lim_{n \to \infty} \abs{\frac{n}{(1 + 2 \eta)^{n}}}\\
&= 0.
\end{align*}

Der Faktor $\frac{\lambda - 1}{2 (2 \lambda - 1)^2}$ in Gleichung \eqref{eq:transport:osz:oh2_B} ist für $\eta > 0$ positiv!
Das heißt, dass sich $z_0$ stabil und diffusiv in der Zeit ausbreitet.
...
Dann gilt
\begin{align*}
\partial_t z_0(t, \tau, x) + \frac {1} {2\lambda - 1} \partial_x z_0(t, \tau, x) &= 0\\
\partial_t z_1(t, \tau, x) + \frac {1} {2\lambda - 1} \partial_x z_1(t, \tau, x) &= \frac{\lambda - 1}{2 (2 \lambda - 1)^2} \partial^2_x z_0(t, \tau, x) - \partial_\tau z_0(t, \tau, x)\\
\end{align*}
und fordert, dass
\[ 
\partial_\tau z_0(t, \tau, x) = \frac{\lambda - 1}{2 (2 \lambda - 1)^2} \partial^2_x z_0(t, \tau, x) 
\]
gilt. Mit $z_1(0,0,x) = 0$ folgt also $z_1 = 0$ und
\begin{align*}
z_0(t, \tau, x) &= (\Delta_\eps U * G_\tau)\left(x - \frac{t}{2 \lambda - 1} \right) \\
G_\tau(y) &= \sqrt {\frac{1}{2 \pi \tau} \frac {(2 \lambda - 1)^2}{(\lambda - 1)}} \exp\left( - \frac{y^2}{2 \pi \tau} \frac {(2 \lambda - 1)^2}{(\lambda - 1)} \right)
\end{align*}
Insgesamt erhalten wir für den Spezialfall $\Delta_\eps U(x) = \eps$
