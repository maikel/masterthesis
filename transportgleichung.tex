...
\end{semilogyaxis}
\end{tikzpicture}
\caption{Hier vergleichen wir die maximalen Fehler der numerischen Lösungen für die Startwerte $U(x) = sin(\pi x)$, $V(x) = \eps_M$ und $h = 10^{-3}$ für $\eta = 0.1, 0.05$ und $0.001$ zur echten Lösung $u(t,x) = sin(\pi(x - t))$ mit dem geschätztem Fehler $u(t_n, x_i) - v^n_i$ in \eqref{eq:transport:regulaer:fehler}.}
...
Mit dem Satz \ref{satz:omega_n} und weil wir in jeder Ordnung die selbe Frequenz nehmen, folgt hiermit, dass für alle $k = 0,1,2$ und für alle $n \in \N$ und $i \in \Z$
...
Setzen wir $\lambda = 1 + \eta$ für ein $\eta > 0$, dann schreiben wir diesen Ansatz hier nun als
...
Insgesamt erhalten wir das Ergebnis
