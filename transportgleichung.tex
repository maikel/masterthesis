...
\begin{tikzpicture}
\begin{semilogyaxis}[
    title={$\norm{\sin\bigl(\pi (x_i - n h (1 + \eta) )\bigr) - v^n_i}_\infty, h = 10^{-3}$},
    xlabel={Iterationen $n$},
    ylabel={maximaler Fehler},
    legend entries={$\eta=0.01$,$\eta=0.05$,$\eta=0.1$,$(1+2 \eta)^n\eps + \frac{\pi^2}{2} n h^2 (1 + \eta) \eta$, $(1+2 \eta)^n\eps$},
...
\partial_t z_2(t, x) + \partial_x z_1(t, x) &= \frac{1}{2} \partial^2_x z_0(t, x) - \partial^2_t z_0(t, x) + 2 z_1(t, x).
\end{align*}
