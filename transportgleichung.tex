...
F(t_n,x_k) = e^{i \omega k} \Omega(n) g(t_n,x_k).
\end{align}
...
\end{align}
erfüllt ist.
Das impliziert wegen $\abs{f(n, k(n), t, x)} - \abs{F(t,x)} \leq \abs{F(t, x) - f(n, k(n), t, x)}$ die Gleichung
...
Im Folgenden betrachten wir immer den Ansatz
...
&= \frac{1}{2} \partial^2_x w_0(t_n, x_k) + e^{i(\omega k - \theta n)} e^{- i\omega} \partial^2_x z_0(t_n, x_k)
\end{align*}
und
\begin{align*}
...
&= -\frac{\lambda}{2} \partial^2_t w_0(t_n, x_k) - e^{i(\omega k - \theta n)} e^{- i\theta} \partial^2_t z_0(t_n, x_k)\\
&= -\frac{\lambda}{2} \partial^2_x w_0(t_n, x_k) - e^{i(\omega k - \theta n)} e^{- i\theta} e^{- 2 (\omega - \theta)} \partial^2_x z_0(t_n, x_k)
\end{align*}
also folgt in Summe
\begin{align}
&\frac {1}{2} \partial^2_x u_0(n, k-1, t_n, x_k) -\frac {\lambda}{2} \partial^2_t u_0(n+1, k, t_n, x_k)\\
&= \frac {1-\lambda}{2} \partial^2_x w_0(t_n, x_k)\\
&\qquad + e^{i(\omega k - \theta n)} \left(  \right)
\end{align}