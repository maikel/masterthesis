...
Wir versuchen hier den instabilen Fall $\lambda > 1$ besser zu verstehen.
...
\end{align}
gilt.
Im Anhang \eqref{appendix:regulaer:rechnungen} werden die Differenzen $v^{n+1}_i - v^n_i$ und $\lambda ( v^n_i - v^n_{i-1} )$ aus \eqref{eq:adv:scheme} ausgerechnet.
In diese Rechnungen geht wesentlich ein, dass $u_0$ und $u_1$ differenzierbar sind.
Sortiert man die Ergebnisse nun nach den Ordnungen von $h$, so erhält man das Gleichungssystem
