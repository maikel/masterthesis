...
Deshalb ist es nützlich und sinnvoll, die richtige Lösung der Gleichung zu bestimmen.
Bei der Lösungskonstruktion hilft uns jedes mal die Methode der Charakteristiken weiter.
...
\begin{align}
\left. D u\bigl(\varphi_t(s)\bigr) \right|_{s = \tau} = \partial_t u(\tau, x + \tau - t) + \partial_x u(\tau, x + \tau - t) = 0.
\end{align}
Insbesondere impliziert das für alle $t \in \Rp$
\begin{align}
...
Andererseits ist jedes so definierte $u(t,x) = U(t - x)$ auch eine Lösung von \eqref{eq:transport:analytisch:homogen}, solange das für $U$, im womöglich schwachem Sinne, möglich ist.

\subsection{Die inhomogene Transportgleichung}
Wir betrachten nun die lineare Transportgleichung mit nichtverschwindender rechter Seite
...
\begin{align}
\left. D u\bigl(\varphi(s)\bigr) \right|_{s = \tau} = F(\tau,x + \tau - t).
\end{align}
Demnach folgt wegen des Hauptsatzes für die Integral und Differentialrechnung
\begin{align}
\begin{split}
...
Dann folgt hieraus $F(\tau, x + \tau - t) = F(0, x-t)$ für alle $\tau \in \Rp$ und somit gilt für alle $t \in \Rp$
\begin{align}
...
\begin{semilogyaxis}[
    ...
    legend entries={$\err^n \, \eta=0.01$,$\err^n \, \eta=0.05$,$\err^n \, \eta=0.1$,$(1+2 \eta)^n\eps_M + n \frac{\pi^2}{2} h^2 (1 + \eta) \eta$, $(1+2 \eta)^n\eps_M$},
