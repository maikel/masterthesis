...
\section{Erster Regulärer Ansatz}

\section{Beispiel für Instabiles Verhalten}\label{sec:transport:beispiel}
...
\section {Ansatz für alternierende Vorzeichen}\label{sec:transport:osz}
...
\begin{semilogyaxis}[
    ...
    xlabel={Zeit $t$},
    ylabel={maximaler Fehler},
    legend entries={$\err^n$,$\eps e^{2t}$, $ \eps e^{2t} + t h^2 \frac{\pi^2}{2}$},
...
Diese PDE hat die Lösung

\begin{align*}
w_2(t,x) &= - t \frac{1}{2} \partial^2_x w_0(t, x) \qquad \text{und}\\
z_2(t,x) &= e^{2t} \int_0^t e^{-2s} \left[ - \frac{1}{2} \partial^2_x z_0(s, x) + 2 \partial_x z_0(s, x) - z_0(s,x) \right] \dd s\\
\end{align*}
